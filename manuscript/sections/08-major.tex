...
+E_q(\mu,\lambda,\delta),
\qquad
|E_q|\;\ll\;\frac{\mu Q}{P}+\frac{\lambda P}{Q}+1
+\delta\Big(\frac{\mu Q}{P^{2}}+\frac{\lambda}{Q}\Big)^{-1},
\]
\end{lemma}
...
($\asymp\mu q/p^2+\lambda/q$) plus $O(1)$ endpoint integers; partial
windows at the range ends contribute the fourth term. Summing the main
terms reconstitutes $2\delta P$; the endpoints and edges give $E_q$
(with $\delta=\delta_\mu$ the edge term sums unconditionally:
$\sum_\mu P^2/(\mu^2Q\Llog^B)\ll P\Llog^{-B}$ and, for $P>Q$,
$\sum_\mu Q/(\mu\lambda\Llog^B)\ll Q\Llog^{1-B}$).
\end{proof}
...
\begin{remark}[diagnosis: only the signed average is needed]
\label{rem:E1-signed}
The Family-A application below sums the nonnegative counts $N_q$, so
E1 is only ever invoked for the \emph{signed} average
$\mathbb E_q\sum_\mu E_q$ --- strictly weaker than the stated absolute
form, and what equidistribution heuristics actually deliver. Moreover,
two of the three boundary pieces are unconditionally acceptable:
$\sum_{\mu\le R}O(1)\ll R\le P$ and
$\sum_{\mu\le R}\lambda P/Q\ll\Llog^{C}RP/Q$ with $R/Q=x^{1-u-2u'}<1$
in the band; only the $\mu Q/P$ branch piece, in the sub-range
$\mu\ge P/Q$, requires the $q$-average. Numerically the signed average
sits at $1$--$4\%$ of the main term, $10$--$300\times$ below the
worst-case bound, in both band geometries
(Appendix~\ref{app:numerics}). Proof route: Erd\H os--Tur\'an expansion
plus lattice counting near the hyperbolas $\mu q\approx kp$; Vaughan
for the prime variable.
\end{remark}
...
term). (The earlier draft's clause on the band average of $\mathcal D_R$ is
withdrawn: for $u>u'$ the structural first gap makes
$\mathbb E[\mathcal D_R]$ power-large; only the per-pair certificate
and the failure count are used downstream.)
\end{proof}
