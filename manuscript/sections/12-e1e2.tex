...
\ll\big(P/\sqrt s+\sqrt{Ps}+P^{4/5}\big)\log^{4}P$.
\emph{(Statement corrected June 11 per the citations audit: the middle
term is $\sqrt{Ps}$, not $\sqrt s$; the downstream uses in this
section already carry the partial/sub-band caveats, and the Family-B
count itself is discharged unconditionally by the wp8-e2 route, which
does not consume SL2.)}
\end{lemma}
